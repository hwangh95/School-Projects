...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        ration = self.numParticles/len(self.legalPositions)
        self.particles = []
        for a in self.legalPositions:
            self.particles.extend([a]*ration)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            if all(v == 0 for v in allPossible.values()):
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        newList = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))) for oldPos in self.particles]
        self.particles = newList 
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        posPermutation = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(posPermutation)

        ration = self.numParticles/len(posPermutation)
        self.particles = []
        while len(self.particles) < self.numParticles:
            for a in posPermutation:
        ...
        for index, part in enumerate(self.particles):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    probabilities *= emissionModels[i][trueDistance]
            allPossible[part] += probabilities
